from cobot.tasks import CobotTasks
import json

class Cobot:
    def __init__(self):
        self.angulo_anterior = CobotTasks.cobot_angles.delay().get()
    ...
    def run_command(self, command, type, data=""):
        if command == "play":
            if type == "point":
                ...
                CobotTasks.cobot_points.delay(command, type, self.angulo_actuales, velocidad)

                self.angulo_anterior = self.angulo_actuales

            elif type == "movement":
                CobotTasks.cobot_movements.delay(command, type, data)

            elif type == "sequence":
                CobotTasks.cobot_sequences.delay(command, type, data)
        
        elif command == "cli":
            if type=="home":
                self.angulo_anterior = [0.0]*5
                CobotTasks.cobot_home_reset.delay()
            elif type == "angles":
                return CobotTasks.cobot_angles.delay().get()
            elif type == "motors_off":
                print("En cobot task")
                CobotTasks.motors_off.delay()
            elif type == "motors_on":
                x = CobotTasks.motors_on.delay().get()
